...
@pytest.fixture()
def client():
    """TestClient used as context manager so the FastAPI lifespan runs."""
...
def test_agent_run_with_user_id(client: TestClient) -> None:
...
def test_agent_run_with_session_id(client: TestClient) -> None:
...
def test_agent_run_missing_input_returns_422(client: TestClient) -> None:
    response = client.post("/agent/run", json={})
    assert response.status_code == 422


def test_agent_run_increments_metrics(client: TestClient) -> None:
